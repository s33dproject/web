...
const sketch = ({ width, height }) => {
        ...
        for (let x = 0; x < count; x++) {
            for (let y = 0; y < count; y++) {
                ...
                points.push({
                    ...
                    character: random.pick(defaultCharacters),
                    position: [u, v]
                });
            }
        }
    ...
    return ({ context, width, height }) => {
        ...
        grid.forEach(({ position, rotation, size, color, character }) => {
            ...
            const charToUse = chars.length > 0 ? random.pick(chars) : character;
            ...
            context.fillText(charToUse, 0, 0);
            context.restore();
        });
    };
};
